...
function mv_entire()
{
	...
	if [ -f $entirefile ]; then
		...
		#如果当天的全量checkfile存在，则说明之前来过一次全量文件了，现在又来，则判断这次来的文件的accesstime($12)和checkfile最新的修改的时间($145)是否相差4个小时
		#如果相差4个小时，则可以说明，和上一次的全量文件不同，是第二次的全量文件
		#如果当天的全量checkfile不存在，则说明今天还没有检测到全量文件，则判断全量文件的数据时间
		if [ -f $checkfile ]; then

			lastcheckdate=`stat -t ${checkfile} |awk '{print $14}'`
			entirefiledate=`stat -t ${entirefile} |awk '{print $12}'`

			ft=$(( $entirefiledate - $lastcheckdate ))
			echo "[`date '+%Y-%m-%d %H:%M:%S'`] $entirefiledate $lastcheckdate $ft" >> $LOG_FILE

			if [[ $ft -gt 14400 ]]; then
				updatetime=`head -n1 $entirefile | awk -F'\t' '{print $4}'`
				dt=`date -d @${updatetime} '+%Y-%m-%d %H:%M:%S'`
				echo "[`date '+%Y-%m-%d %H:%M:%S'`] $entirefile $updatetime" >> $checkfile
			...
			dt=`date -d @${updatetime} '+%Y-%m-%d %H:%M:%S'`
			echo "[`date '+%Y-%m-%d %H:%M:%S'`] $nowtime $updatetime $ft" >> $LOG_FILE
