...
file=""
logpath="/data0/mrjx/topsale/logs/"
...
function insertTopsale1
{
    ...
    echo "[`date '+%Y-%m-%d %H:%M:%S'`] begin" >> ${logfile} 
...
function getTopsale
{
    echo "[`date '+%Y-%m-%d %H:%M:%S'`] getTopsale begin" >> ${logfile}
    if [ $# -eq 0 ]
    then
        dt=`date '+%Y-%m-%d' --date="-1 day"`
        filedt=`date '+%Y%m%d' --date="-1 day"`
    ...
    hql="
        select item_sku_id, del_flag, salenum from $tablename where dt='$dt' and salenum>=10 sort by salenum desc
        "
    echo ${hql} >> ${logfile}
...
function uploadjss
{
    #bindir="/data0/mrjx/topsale/bin/"
	#cd $bindir
	./uploadjss ${file}
   echo "[`date '+%Y-%m-%d %H:%M:%S'`] ./uploadjss ${file} success" >> ${logfile}
}
